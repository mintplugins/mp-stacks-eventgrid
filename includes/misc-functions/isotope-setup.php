 ...
 *
 * To use for your own Add-On, find and replace "eventgrid" with your plugin's prefix
 *
 * @since 1.0.0
 *
...
/**
 * Add the meta options for "Isotope" to the EventGrid Metabox
 *
 * @access   public
 * @since    1.0.0
 * @param    Void
...
*/
function mp_stacks_eventgrid_add_isotope_meta( $items_array ){
...
add_filter( 'mp_stacks_' . 'eventgrid' . '_items_array', 'mp_stacks_eventgrid_add_isotope_meta', 13 );
...
*/
function mp_stacks_eventgrid_isotope_filter_group_options( $isotope_filter_groups ){
	
	//This array can contain custom groups (for outside sources like instgram), AND/OR WordPress taxonomy slugs.			
	$isotope_filter_groups = mp_stacks_eventgrid_isotope_filter_groups();
...
add_filter( 'eventgrid' . '_isotope_filter_groups', 'mp_stacks_eventgrid_isotope_filter_group_options' );
 ...
 * @return   Array - Returning out it is an array containing the Filter Groups that the user can choose from.
*/
function mp_stacks_eventgrid_isotope_filter_groups(){
	...
	$isotope_filter_groups = apply_filters( 'mp_stacks_eventgrid_isotope_filter_groups', $isotope_filter_groups );
...
*/
function mp_stacks_eventgrid_all_icon( $isotope_icon, $meta_prefix ){
